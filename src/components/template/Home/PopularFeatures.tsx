...
export default function PopularFeatures() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/popularFeaturesItems").then((res) => {
      ...
      const uniqueCategories= [...new Set(result.map((item:CheapestHousesItem) => item.status))];
      setStatuses(uniqueCategories);
    });
  }, []);

  const filterItem = (status: string) => {
    let newFilteredItems = data.filter((item:CheapestHousesItem) => item.status === status);
